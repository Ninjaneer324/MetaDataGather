...
for elem in periodic_table:
    ...
    for a in alloys:
        ...
        query = "(("+periodic_table[a]['name']+" OR "+a+") AND (precipitate AND "+"(age OR transform OR microscope)))"
        worksheet.write(row, 0, elem+"-"+a)
        print(query)
        #requests for search results
        response = requests.get(url, headers=headers,params={"query":query,"count":200,"sort":"relevance"}) #engineering village doesn't have count
